...
func GetUserTokens(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	tokens, err := model.GetAllUserTokens(id, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	if err != nil {
		common.ApiError(c, err)
		return
	}
	total, _ := model.CountUserTokens(id)
	pageInfo.SetTotal(int(total))
...
func AdminClearUserBinding(c *gin.Context) {
	...
	model.RecordLog(user.Id, model.LogTypeManage, fmt.Sprintf("admin cleared %s binding for user %s", bindingType, user.Username), otherParam)
	...
	case "add_quota":
		...
		case "add":
			...
			model.RecordLog(user.Id, model.LogTypeManage,
				fmt.Sprintf("管理员(%s)增加用户额度 %s", adminName, logger.LogQuota(req.Value)), otherParam)
		case "subtract":
			if req.Value <= 0 {
				common.ApiErrorI18n(c, i18n.MsgUserQuotaChangeZero)
				return
			...
			model.RecordLog(user.Id, model.LogTypeManage,
				fmt.Sprintf("管理员(%s)减少用户额度 %s", adminName, logger.LogQuota(req.Value)), otherParam)
		case "override":
			oldQuota := user.Quota
			...
			model.RecordLog(user.Id, model.LogTypeManage,
				fmt.Sprintf("管理员(%s)覆盖用户额度从 %s 为 %s", adminName, logger.LogQuota(oldQuota), logger.LogQuota(req.Value)), otherParam)
		default:
			common.ApiErrorI18n(c, i18n.MsgInvalidParams)
			return
		}
