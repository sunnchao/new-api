...
function parseUserUnUsableMap(value: string): Record<string, string> {
  return safeJsonParse<Record<string, string>>(value, {
    fallback: {},
    silent: true,
  })
...
function buildGroupPricingRows(
  ...
  const unselectableMap = parseUserUnUsableMap(userUnselectableGroups)
  const names = new Set([
  ...
  return [...names].map((name) => ({
    ...
    hidden: Object.hasOwn(unselectableMap, name),
...
function sourceGroupPricingSignature(
  ...
  return JSON.stringify({
    ...
    userUnselectableGroups: parseUserUnUsableMap(userUnselectableGroups),
  })
}
...
function GroupPricingTable({
  ...
  const [rows, setRows] = useState<GroupPricingRow[]>(() =>
    buildGroupPricingRows(groupRatio, userUsableGroups, topupGroupRatio, userUnselectableGroups)
  )
  ...
  }, [groupRatio, userUsableGroups, topupGroupRatio, userUnselectableGroups])
          ...
          <StaticDataTable
            ...
            columns={[
              ...
              },
              {
                ...
                className: 'min-w-56',
                cell: (row) => (row.selectable || row.hidden) ? (
                    <Input
