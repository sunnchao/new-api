...
function ModelRow(props: { model: PricingModel }) {
  ...
  return (
    <Link
      href={`/pricing/${encodeURIComponent(props.model.model_name)}`}
...
export function PublicPlanModelsDialog(props: PublicPlanModelsDialogProps) {
  ...
  const pricingHref = getSingleGroupPricingHref(props.groups)
      ...
      <DialogContent className='max-h-[90vh] max-sm:w-[calc(100vw-1.5rem)] sm:max-w-2xl'>
        ...
        <div className='flex flex-wrap gap-1.5'>
          {props.groups.map((group) => (
            <Link
              key={group}
              href={`/pricing?group=${encodeURIComponent(group)}`}
              className='w-fit'
            >
        ...
        <DialogFooter>
          <Button render={<Link href={pricingHref} />}>
